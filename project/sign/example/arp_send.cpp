...
void arp_request(const char *if_name, const char *dst_ip)
{
    ...
    memcpy(eth_header->ether_shost, src_mac_addr, ETH_ALEN);    //源以太网地址
    memcpy(eth_header->ether_dhost, dst_mac_addr, ETH_ALEN);    //目的以太网地址
    eth_header->ether_type = htons(ETHERTYPE_ARP);              //0x0806
